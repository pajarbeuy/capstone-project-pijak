...
def fix_slangwords(text: str) -> str:
    fixed_words = [slangwords.get(word.lower(), word) for word in str(text).split()]
...
def get_stopwords() -> set[str]:
    ...
    if stopwords is not None:
        try:
            words.update(stopwords.words("indonesian"))
...
def preprocess_text(text: str) -> str:
    ...
    stemmed = stemming_text(fixed)
    tokens = tokenizing_text(stemmed)
    filtered = filtering_text(tokens)
    return to_sentence(filtered)


# Backward-compatible names used by older website code and notebook-01 wording.
clean_text = cleaning_text
normalize_text = fix_slangwords
remove_stopwords = lambda text: to_sentence(filtering_text(str(text).split()))
stem_text = stemming_text
